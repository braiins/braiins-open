...
impl TestBlock {
    ...
    pub fn new(
        ...
        Self {
            hash: DHash::from_hex(hash).expect("BUG: parse hex"),
            hash_str: hash,
            midstate: Midstate::from_hex(midstate).expect("BUG: parse hex"),
            midstate_str: midstate,
            version,
            previous_hash: DHash::from_hex(previous_hash).expect("BUG: parse hex"),
            merkle_root: DHash::from_hex(merkle_root).expect("BUG: parse hex"),
            time,
            bits,
            target: Target::from_compact(bits).expect("BUG: network difficulty"),
            nonce,
            header_bytes,
            icarus_bytes,
        }
