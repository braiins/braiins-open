...
fn build_signed_message(signed_part: SignedPart, signature: ed25519_dalek::Signature) -> Bytes {
...
async fn main() -> anyhow::Result<()> {
    ...
    let (signed_part, _, static_keypair, sig) = build_deterministic_signed_part_and_auth();
    let signature_noise_message = build_signed_message(signed_part, sig);
    let tcp_listener = TcpListener::bind(TEST_SERVER_SOCKET).await?;
    let responder = Responder::new(
        &static_keypair,
        signature_noise_message,
        vec![EncryptionAlgorithm::ChaChaPoly, EncryptionAlgorithm::AESGCM],
    );
    let (downstream, _) = tcp_listener.accept().await?;
